...
import ShimmerPostCard from "../Posts/ShimmerPostCard";
      ...
      try {
        ...
        if (activeTab === "Your Blog") {
          response = await axios.get(
            `http://localhost:8080/api/v1/user/${currentUserId}/posts`
          );
        } else if (activeTab === "Liked Blogs") {
          response = await axios.get(
            `http://localhost:8080/api/v1/user/${currentUserId}/liked/posts`
          );
        } else if (activeTab === "Commented Blogs") {
          response = await axios.get(
            `http://localhost:8080/api/v1/user/${currentUserId}/commented/posts`
          );
        }
    ...
    <div className="flex flex-col min-h-screen p-6">
      ...
      <div className="flex flex-1">
            ...
            {tabs.map((tab) => (
              <button
                ...
                className={`block w-full text-left px-4 py-2 rounded-lg transition duration-200 ${
                  activeTab === tab
                    ? "bg-white text-[#4E3A59] font-semibold shadow-md"
                    : "hover:bg-[#6A4A6F]"
                }`}
        ...
        <div className="w-3/4 bg-white p-6 ml-6 shadow-lg rounded-lg border border-gray-300">
          <h2 className="text-2xl font-semibold text-[#4E3A59] mb-4">
            {activeTab}
          </h2>
